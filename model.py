...
#       #label = tf.reshape(tf.transpose(label),[-1,])
#       hit = 0
#       total = 0
#       dim =int(pred.shape[0])
#       for i in range(int(dim / 7)):
#           ok = True
#           for j in range(7):
#               k = i * 7 + j
#               if tf.argmax(pred[k],axis=0) != label[k]:
#                   ok = False
#                   break
#           if ok:
#               hit += 1
#           total += 1
#           accuracy = tf.cast(1.0 * hit / total,tf.float32)
#           tf.summary.scalar(scope.name+'/accuracy', accuracy)
#       return accuracy
